...
import { AddAnimeDTO } from "./AddDTO.ts";

export class AddGenresInAnimeUseCase {
    constructor(private mongoAnimeRepository: MongoAnimeRepository) {}
  
    async execute(data: AddAnimeDTO): Promise<boolean | string> {
        try {
            ...
            if (dataAnime.genres) {
              const dataGenres: string[] = [...dataAnime.genres, data.nameGenre.trim()];
