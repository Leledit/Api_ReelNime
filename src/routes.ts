...
import { listByGenreController } from "./useCases/Dasboard/ListByGenre/index.ts";
const router = Router();
...
router.get(
  "/dashboard/genre",
  listByGenreController.validateRequest,
  (request, response) => {
    return listByGenreController.handle(request, response);
  }
);
